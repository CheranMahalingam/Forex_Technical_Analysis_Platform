...
def cpi_preprocess():
    ...
    australia_cpi_data = pd.read_csv(
        "lstm_model/data/external/cpi/AUD_CPI_2018-2020.csv"
    )
    new_zealand_cpi_data = pd.read_csv(
        "lstm_model/data/external/cpi/NZD_CPI_2018-2020.csv"
    )
    ...
    aud_cpi["Time"] = aud_cpi["Time"].transform(
        lambda time: datetime.strptime(time[:4] + "20" + time[4:], "%b-%Y").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    )
    ...
    create_cpi_csv(cad_cpi, time_frame, cad_cpi["CPI"].iloc[0]).to_csv(
